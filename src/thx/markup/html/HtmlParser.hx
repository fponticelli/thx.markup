...
import thx.core.Set;
import thx.markup.html.HtmlHandler;
...
class HtmlParser {
	...
	{
		if(null == html) throw 'null argument "html"';
		this.html = html;
	}
	...
	{
		if(null == handler) throw 'null argument "handler"';
		...
		while (html != '') {
			...
			if (stacklast() == null || !Element.isSpecial(stacklast()))
			{
				...
				if (html.indexOf("<!--") == 0)
				{
					index = html.indexOf("-->");
					if(index >= 0) {
						handler.comment(html.substr(4, index - 4));
